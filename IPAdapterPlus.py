...
class DREIPAdapterPrepare:
    ...
    RETURN_TYPES = ("PREPARED_IPADAPTER",)
    FUNCTION = "prepare_ipadapter"
    CATEGORY = "ipadapter"

    def prepare_ipadapter(self,
                          ipadapter,
                          clip_vision=None,
                          image=None,
                          noise=None,
        ...
        self.device = comfy.model_management.get_torch_device()
        self.is_full = "proj.3.weight" in ipadapter["image_proj"]
        self.is_portrait = "proj.2.weight" in ipadapter["image_proj"] and not "proj.3.weight" in ipadapter[
            "image_proj"] and not "0.to_q_lora.down.weight" in ipadapter["ip_adapter"]
        self.is_faceid = self.is_portrait or "0.to_q_lora.down.weight" in ipadapter["ip_adapter"]
            ...
            if self.is_faceid:
                ...
                for i in range(face_img.shape[0]):
                    for size in [(size, size) for size in range(640, 128, -64)]:
                        ...
                        if face:
                            face_embed.append(torch.from_numpy(face[0].normed_embedding).unsqueeze(0))
                            face_clipvision.append(NPToTensor(
                                insightface_face_align.norm_crop(face_img[i], landmark=face[0].kps, image_size=256)))
        ...
        patch_kwargs = []
        if is_tiled:
            for i in range(image_prompt_embeds.shape[
                               0]):  # TODO: check if we can apply one mask per image in the attention patch phase
                patch_kwargs.append({
                    "ipadapter": self.ipadapter,
                    "cond": image_prompt_embeds[i].unsqueeze(0),
                    "uncond": uncond_image_prompt_embeds[i].unsqueeze(0),
        ...
        return (patch_kwargs, attn_mask, image, self.is_sdxl),


class DREIPAdapterApply:
    @classmethod
    def INPUT_TYPES(s):
        return {
            "required": {
                "prepared_ipadapater": ("PREPARED_IPADAPTER",),
                "model": ("MODEL",),
                "weight": ("FLOAT", {"default": 1.0, "min": -1, "max": 3, "step": 0.05}),
                "weight_type": (["original", "linear", "channel penalty"],),
                "start_at": ("FLOAT", {"default": 0.0, "min": 0.0, "max": 1.0, "step": 0.001}),
    ...
    def apply_ipadapter(self,
        ...
        patch_kwargs, attn_mask, image, is_sdxl = prepared_ipadapater
        self.is_sdxl = is_sdxl
        self.work_model = model.clone()
        ...
        for patch_kwarg in patch_kwargs:
            patch_kwarg["weight"] = weight
            patch_kwarg["weight_type"] = weight_type
            patch_kwarg["sigma_start"] = sigma_start
            patch_kwarg["sigma_end"] = sigma_end
            patch_kwarg["number"] = 0
            self.apply_patch(patch_kwarg)
...
NODE_CLASS_MAPPINGS = {
    ...
    "DREIPAdapterPrepare": DREIPAdapterPrepare,
    "DREIPAdapterApply": DREIPAdapterApply,
}
...
NODE_DISPLAY_NAME_MAPPINGS = {
    ...
    "DREIPAdapterPrepare": "DRE💩- IPAdapterPrepare",
    "DREIPAdapterApply": "DRE💩- IPAdapterApply",
}
